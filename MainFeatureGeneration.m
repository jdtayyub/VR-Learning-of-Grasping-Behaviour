...
classes = {};
allData = [];
for person = 3 : length(files) % Looping through People
   tabread = readtable([path '/' files(person).name '/S001/trial_results.csv']);
   ...
   objPos = tabread.position_rel; % Obstacle / Block
   %Add condition filter whether random or not 
  
   % x1-x4 coordinate of the left gap and the right gap around teh obstacle-
   % Plotted to confirm
   ...
   feat = {};
   for i = 1 : length(x1)
     feat(i,:) = featureGenerationFor2Rec([x1(i) 0 x2(i) 0], [x3(i) 0 x4(i) 0]); % Y coordinate are currently set to 0
   end
